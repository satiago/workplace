...
if(isset($_POST['title'])){
	...
	if(isset($_SESSION["unc"])){
		$sql=mysql_query("select id from tb_user where username='".$_SESSION["unc"]."'");
		$info=mysql_fetch_array($sql);
		$userid=$info['id'];
	}
	if(isset($_SESSION['userword'])){
		$sql=mysql_query("select id from tb_admin where adminname='".$_SESSION["userword"]."'");
		$info=mysql_fetch_array($sql);
		$userid=$info['id'];
	}

	$createtime=date("Y-m-d H:i:s");		//获取回复留言时间
	if(mysql_query("insert into tb_replyword (userid,createtime,title,content,leave_id)values('$userid','$createtime','".$_POST['title']."','".$_POST['content']."','".$_POST['t_id']."')")){
