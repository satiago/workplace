...
$sql1=mysql_query("select username,face,ip,email,qq from tb_user where id='".$infoes["userid"]."'",$conn);
...
if($adm!="")
{
$adms=mysql_query("select id from tb_admin where adminname='".$adm."'",$conn);
$re=mysql_fetch_array($adms);
if($re['id']==$infoes['userid'])
{
?>
...
}else if($user== $info1['username'] and $user!=""){ 
?>
  	<td width="50" background="images/dh_back_1.gif"><div align="center"><a href="index.php?ed=<?php echo $infoes['id'];?> & id=<?php echo urlencode("编辑留言");?>">编辑</a></div></td>
<?php
}else{
...
$sql1=mysql_query("select username,face,ip,email,qq from tb_user where id='".$infoes["userid"]."'",$conn);
$info1=mysql_fetch_array($sql1); 
if($info1["face"]=="" && $info1["username"]==""){
?>
<img src="images/gly.gif" /><br>管理员
<?php 
}else{
?>				
<img src="<?php echo $info1["face"]; ?>" /><br><?php echo $info1["username"];?></div></td>
<?php 
}
?>	
...
if($total==0){
	...
	while($info=mysql_fetch_array($sql)){
		--$i;
		$sqlreply=mysql_query("select username,face,ip,email,qq from tb_user where id='".$info["userid"]."'",$conn);
		  ...
		  <table width="550" height="22" border="0" align="center" cellpadding="0" cellspacing="0">
            <tr>
              <td width="300" height="22" background="images/dh_back_1.gif">&nbsp;
                回&nbsp;&nbsp;复：<?php echo unhtml($info["title"]);?>
              <div align="left"></div></td>
<?php
if($adm!="")
{
$admes=mysql_query("select id from tb_admin where adminname='".$adm."'",$conn);
...
}else if($user== $inforeply['username'] and $user!=""){ 
?>
	<td width="50" background="images/dh_back_1.gif"><div align="center"><a href="index.php?ed=<?php echo $info['id'];?> & id=<?php echo urlencode("回复编辑留言");?>">编辑</a></div></td>
<?php
}else{ 
...
<?php 
}elseif($user==$inforeply['username'] and $user!="" ){
...
?>
		  ...
		  <table width="550" height="120" border="0" align="center" cellpadding="0" cellspacing="0">
             <tr>
                ...
                <td width="150" height="90"><div align="center">
<?php 
$sqlr=mysql_query("select username,face,ip,email,qq from tb_user where id='".$info["userid"]."'",$conn);
$infor=mysql_fetch_array($sqlr); 
if($infor["face"]=="" && $infor["username"]==""){
?>
<img src="images/gly.gif" /><br>管理员（<?php echo $i;?>楼）
<?php 
}else{
?>				
<img src="<?php echo $infor["face"]; ?>" /><br><?php echo $infor["username"];?>（<?php echo $i;?>楼）
<?php 
}
?>
</div></td>
