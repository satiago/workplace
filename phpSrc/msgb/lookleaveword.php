			 ...
			 if($total==0){					//如果总留言条数为0，则给出提示
			  echo "<div align=center>对不起，暂无留言！</div>";
			   ...
			   while($info=mysql_fetch_array($sql)){			//通过while循环显示所有留言
				 ...
				 $sql1=mysql_query("select username,face,ip,email,qq from tb_user where id='".$info["userid"]."'",$conn);		//通过传递的id值查询当前留言者的个人信息
...
if($adm!="")
{
$adms=mysql_query("select id from tb_admin where adminname='".$adm."'",$conn);			//从管理员数据表中查询登录的版主id
...
<?php
	...
	}else if($user== $info1['username'] and $user!=""){       //如果当前用户是该留言的发表者，则显示编辑按钮
?>
  <td width="50" background="images/dh_back_1.gif"><div align="center"><a href="index.php?ed=<?php echo $info['id'];?> & id=<?php echo urlencode("编辑留言");?>">编辑</a></div></td>
<?php
	}else{
...
?>
			...
			<?php
			}else if($user!= $info1['username'] and $user!=""){      //当前用户不是该帖子的发表者时，可以对帖子进行回复
			?>
			 <td width="98" background="images/dh_back_1.gif"><a href="reply.php?t_id=<?php echo $info['id'] ?>">回复此贴</a></td>
			<?php 
			}else{
		...
		<td width="98" background="images/dh_back_1.gif">回复此贴</td>
		  ...
		  <table width="550" height="120" border="0" align="center" cellpadding="0" cellspacing="0">
             <tr>
				...
				<?php 
				if($info1["face"]=="" && $info1["username"]==""){			//当登录者的头像和用户名为空时，显示管理员头像即登录者为管理员
				?>
				...
				?>
				<img src="<?php echo $info1["face"]; ?>" /><br><?php echo $info1["username"];?>
				<?php 
				}
				?>
				</div></td>
